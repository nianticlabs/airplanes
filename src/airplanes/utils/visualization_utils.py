...
from airplanes.utils.generic_utils import reverse_imagenet_normalize

PLANES_COLORMAP = torch.rand((3, 32))


def colormap_planes(plane_ids_hw: torch.Tensor, mask_hw: torch.Tensor = None) -> torch.Tensor:
    plane_ids_hw = plane_ids_hw.cpu().detach().long()
    if mask_hw is None:
        mask_hw = torch.ones_like(plane_ids_hw)
    planes_viz = PLANES_COLORMAP[:, plane_ids_hw]
    planes_viz[:, ~mask_hw.bool()] = 0
    return planes_viz


def instance_ids_from_mesh(mesh) -> np.ndarray:
    """Given a mesh, where vertex colors correspond to plane labels, this function
    returns an array of instance ids for each vertex
    """
    cols = mesh.visual.vertex_colors
    _, pred_instance_ids = np.unique(cols, axis=0, return_inverse=True)
