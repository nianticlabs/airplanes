    ...
    def __init__(
        ...
        if inplanes == planes * self.expansion and stride == 1:
            ...
            self.downsample = nn.Sequential(
                conv(inplanes, planes * self.expansion, bias=bias, stride=stride),  # type: ignore
...
class TensorFormatter(nn.Module):
    ...
    def _expand_batch_with_channels(self, x):
        if x.dim() != 5:
            raise ValueError(
                "TensorFormatter expects tensors with 5 dimensions, " "not {}!".format(len(x.shape))
            )
    ...
    def _reduce_batch_to_channels(self, x):
        if self.batch_size is None or self.depth_chns is None:
            raise ValueError(
                "Cannot  call _reduce_batch_to_channels without first calling"
...
class ConvBlock(nn.Module):
    def __init__(
        ...
        self.non_lin: Union[nn.ReLU, nn.ELU] = nn.ReLU(inplace=True)
        if use_elu:
...
class ConvUpsampleAndConcatBlock(nn.Module):
    def __init__(self, in_ch: int, out_ch: int, skip_chns: int, use_elu: bool = True):
        super(ConvUpsampleAndConcatBlock, self).__init__()

        self.pre_concat_conv = ConvBlock(in_ch=in_ch, out_ch=out_ch, use_elu=use_elu)
        self.post_concat_conv = ConvBlock(in_ch=out_ch + skip_chns, out_ch=out_ch, use_elu=use_elu)
