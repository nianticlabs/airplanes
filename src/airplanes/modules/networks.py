...
from airplanes.modules.layers import BasicBlock
from airplanes.utils.generic_utils import upsample
...
class HarmonicEmbedding(torch.nn.Module):
    def __init__(self, n_harmonic_functions=24, omega0=0.1):
        super().__init__()
...
class WorldEmbeddingEncoder(nn.Module):
    def __init__(self, num_embedding_dims, num_inputs=3, num_harmonics=12):
        super().__init__()
        self.num_embeddings_dims = num_embedding_dims
        self.num_harmonics = num_harmonics

        self.harmonic_embeddor = HarmonicEmbedding(num_harmonics)

        # 1x1 convs in succession
        self.net = nn.Sequential(
            nn.Conv2d(num_embedding_dims + num_harmonics * num_inputs * 2, 64, 1),
            nn.LeakyReLU(),
            nn.Conv2d(64, 64, 1),
            nn.LeakyReLU(),
            nn.Conv2d(64, num_embedding_dims, 1),
        )

    def forward(self, embedding_bdhw, xyz_b3hw):
        xyz_bdhw = self.harmonic_embeddor(xyz_b3hw.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        embedding_b1hw = torch.cat((embedding_bdhw, xyz_bdhw), dim=1)
        return self.net(embedding_b1hw)
...
class DepthDecoderPP(nn.Module):
    def __init__(self, num_ch_enc, scales=range(4), num_output_channels=1, use_skips=True):
        ...
        if len(num_ch_enc) < 5:
            raise ValueError(f"Expected at len(num_ch_enc) >=5, but it is {len(num_ch_enc)}")
